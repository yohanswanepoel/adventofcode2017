...
def start_game():
    ...
    highest_value = get_highest_value_register(register_bank)

    final_register_bank = do_calculations(register_bank,lines)
    get_highest_value_register(final_register_bank)
    print ("Highest ever value",highest_value)

def get_highest_value_register(final_register_bank):
    highest_val = max(final_register_bank.items(),key=operator.itemgetter(1))[1]
...
def perform_action(register_bank,instruction):
    ...
    if register_value > highest_value:
